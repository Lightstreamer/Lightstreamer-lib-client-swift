    ...
    func testReconnectTimeout_NoRecovery_CreateWS() {
        ...
        asyncAssert {
            XCTAssertEqual("""
                ...
                wsok
                create_session\r
                LS_keepalive_millis=5000&LS_cid=\(LS_TEST_CID)&LS_old_session=sid&LS_send_sync=false&LS_cause=ws.stalled
                """, self.io.trace)
            XCTAssertEqual("""
                CONNECTING
                CONNECTED:WS-STREAMING
    ...
    func testReconnectTimeout_NoRecovery_BindHTTP() {
        ...
        asyncAssert {
            XCTAssertEqual("""
                ...
                http.dispose
                http.send http://server/lightstreamer/create_session.txt?LS_protocol=\(TLCP_VERSION)
                LS_polling=true&LS_polling_millis=0&LS_idle_millis=0&LS_cid=\(LS_TEST_CID)&LS_old_session=sid&LS_cause=http.stalled
                """, self.io.trace)
            XCTAssertEqual("""
                CONNECTING
                CONNECTED:STREAM-SENSING
