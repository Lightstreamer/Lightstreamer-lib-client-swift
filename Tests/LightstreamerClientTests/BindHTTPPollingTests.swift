    ...
    func testREQERR_Retry() {
        ...
        asyncAssert {
            XCTAssertEqual(self.preamble + """
                ...
                wsok
                create_session\r\nLS_cid=cid&LS_old_session=sid&LS_send_sync=false&LS_cause=http.reqerr.20
                """, self.io.trace)
            XCTAssertEqual(self.delegatePreamble + """
                CONNECTED:HTTP-POLLING
                DISCONNECTED:WILL-RETRY
    ...
    func testIdleTimeout_in_900_NoRecovery() {
        ...
        asyncAssert {
            XCTAssertEqual(self.preamble + """
                ...
                http.dispose
                http.send http://server/lightstreamer/create_session.txt?LS_protocol=\(TLCP_VERSION)
                LS_polling=true&LS_polling_millis=0&LS_idle_millis=0&LS_cid=cid&LS_old_session=sid&LS_cause=http.idle.timeout
                """, self.io.trace)
            XCTAssertEqual(self.delegatePreamble + """
                DISCONNECTED:WILL-RETRY
                CONNECTING
